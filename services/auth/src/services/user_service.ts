...
import { Auth, User, TokenBody } from "../models";
import messages from "../constants";
import PGPool from "../helpers/db_pool";
...
export class UserService extends CommonService {
  ...
  public static async createToken(user: User): Promise<object> {
    const permissionsResult = await UserService.getPermissions(
      user.username || ""
    );
    ...
    const claims = {
      sub: user.username,
      iss: "https://rafaeljesusaraiva.pt",
      sbxUser: user,
      sbxPermissions: _sbxPermisisons,
    };

    const authObj: Auth = new Auth(
      config.server.apiUuid,
      config.server.tokenExpiration,
      user
    );
  ...
  public async addUser(user: User, pool?: PGPool): Promise<any> {
    ...
    try {
      ...
      const sql_user = `INSERT INTO users (username, salt, hashpass)
				VALUES ('${user.username}', 'salt', '${user.hashedPassword}') returning id`;
      ...
      const userRoleResult = await pool.aquery(
        this.user_current,
        sql_user_roles,
        user_role_params
      );
  ...
  public static async getUserAndAuthToken(
    ...
    try {
      ...
      if (query_results.rowCount <= 0) {
        throw {
          message: messages.errors.user.invalidUserPassword,
          status: 400,
        };
      }
  ...
  public static async forgotPassword(_username: string) {
    ...
    const userDetails = await pool.aquery(objSysAdmin, sql, [
      _username,
      _username,
    ]);
    ...
    try {
      ...
      const updatedUser = await pool.aquery(objSysAdmin, sqlSet, [
        newPassword,
        username,
      ]);
  ...
  public async changePassword(
    ...
    const userDetails = await pool.aquery(this.user_current, sqlGet, [
      _username,
      oldPassword,
    ]);
    ...
    if (userDetails.rowCount <= 0) {
      return {
        success: false,
        data: { message: messages.errors.user.passwordNotMatched },
      };
    }
  ...
  public async updateUser(user: User) {
    ...
    try {
      ...
      if (user.hashedPassword)
        user_columns += `, hashpass = '${user.hashedPassword}'`;
      // update users
  ...
  public async getAllRoles(): Promise<any> {
    return await this.getRows(
      "select id, name, label, description, rank, active from roles",
      []
    );
  }
}
