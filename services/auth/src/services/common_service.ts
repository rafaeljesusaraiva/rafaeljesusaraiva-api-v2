...
export class CommonService {
  ...
  public async insertRow(
    ...
    try {
      ...
      if (result.rowCount === 0)
        throw { message: messages.errors.insert, status: 400 };
      return {
        success: true,
        data: { result: messages.success.insert, id: result.rows[0].id },
      };
    } catch (error: any) {
  ...
  public async insertRows(
    ...
    try {
      ...
      return {
        success: true,
        data: { result: messages.success.insert, ids: result.rows },
      };
    } catch (error: any) {
      console.log(`CommonService.insertRow() Query: ${sql}`);
  ...
  public async getAllRows(table_name: string | null = null): Promise<any> {
    return this.getRows(
      "SELECT * FROM  " + (table_name ? table_name : this.type_name)
    );
  }

  public getFilteredRows(condition: string, params: Array<any>): any {
    return this.getRows(
      `SELECT * FROM ${this.type_name} WHERE ${condition}`,
      params
    );
  }
  ...
  public getBy(_columnName: string, _value: any): Promise<any> {
    return this.getRows(
      `SELECT * FROM ${this.type_name} WHERE ${_columnName} = $1 `,
      [_value]
    );
  }
}
